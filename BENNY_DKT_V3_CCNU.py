...
data_cross_all
    ...
    set_seed(seed)
    # Data实例化:test_data
    test_data = Data(open('./my_data/%s/ccnu_8_math_test.csv' % dataset, 'r'), length, q_num, is_test=True)
    ...
    for cv in range(cv_num):
        ...
        train_data = Data(open('./my_data/%s/ccnu_8_math_train.csv' % dataset, 'r'), length, q_num, is_test=False,
                          index_split=index_split, is_train=True)
        valid_data = Data(open('./my_data/%s/ccnu_8_math_train.csv' % dataset, 'r'), length, q_num, is_test=False,
...
parser.add_argument('--data_cross_all', type=int, default=190, help='')
# 参数中加入args=[]
